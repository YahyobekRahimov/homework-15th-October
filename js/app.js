...
// Maosh
function employeeSalaryRange(salary1, salary2, salary3) {
    let firstPossibility = Math.abs(salary1 - salary3);
    let secondPossibility = Math.abs(salary1 - salary2);
    let thirdPossibility = Math.abs(salary2 - salary3);
    let range = firstPossibility;
    if (secondPossibility > firstPossibility) {
        range = secondPossibility
    }
    if (thirdPossibility > range) {
        range = thirdPossibility;
    }
    return range;
}

// Uchburchakli sonlar
function triangularNumbers(triangleLength) {
    let dotCounter = 0; 
...
// Omadsiz chipta
function unluckyTicket(ticketNumber) {
    let lastTwoDigits;
